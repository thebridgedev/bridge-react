...
`<FeatureFlag>` takes the same context as a `context` prop instead of a third argument — see [Show or hide UI](/feature-flags/using/show-hide-ui/#sending-context) for the component form. Unlike svelte, React needs no getter function for reactivity: both `useFlag`'s third argument and `<FeatureFlag context={{ ... }}>` are plain values re-read on every render, so a new object each render is fine — `useFlag` diffs the resolved value before re-rendering.
