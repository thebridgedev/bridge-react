...
You can observe the connection yourself: `useRealtimeStatus()` is a React hook
returning the current `ConnectionState`, which is one of `'idle'`,
`'connecting'`, `'open'`, or `'closed'`. Read it in a component:
...
> **Framework note:** a store-contract `realtimeStatus` (`{ subscribe }`) is
> exported alongside the hook for code outside components.
