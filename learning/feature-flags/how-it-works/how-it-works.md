...
- **No network on read.** `useFlag` / `<FeatureFlag>` evaluate against an
  in-memory rule cache; there's no request per flag check. `useFlag` subscribes
  to the flag change-bus with `useSyncExternalStore`, so a component only
  re-renders when its resolved value actually changes.
