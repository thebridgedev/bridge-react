...
`useSubscription()` is a hook that returns the checkout-flow state. Call `loadSubscription()` to populate it.

In a component, read it reactively with the hook:

```tsx
import { useEffect } from 'react';
import { loadSubscription, useSubscription } from '@nebulr-group/bridge-react';
...
In plain `.ts` code hooks aren't available (and a destructured hook result doesn't stay reactive); take a one-off imperative read with `useBridgeStore.getState()`:

```ts
import { loadSubscription, useBridgeStore } from '@nebulr-group/bridge-react';
...
```ts
import { getBridgeAuth, loadSubscription } from '@nebulr-group/bridge-react';
```
