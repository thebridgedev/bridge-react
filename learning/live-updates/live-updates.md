...
Every field is a store-contract readable (`{ subscribe }`); the `useBridgeReadable` hook adapts one to React state. They are `null` until the channel delivers the first snapshot; gate on null for skeletons, or fall back to defaults. The `bridge` object's identity is stable; destructure and pass sub-references freely.

> **Framework note:** `useBridge()` returns the same `bridge` singleton for
> hook-style access, and every readable's `subscribe(fn)` (immediate call plus
> a call on every change, returning an unsubscribe function) works from plain
...
The live channel's connection state is exposed as a hook from the flags entry point:

```tsx
import { useRealtimeStatus } from '@nebulr-group/bridge-react/flags';
...
### Relationship to the classic hooks

The `bridge` object and the original hooks (`useAuth`, `useProfile`, `useSubscription`, `useBridgeToken`, ...) are both supported and fed by the same internal state. The `bridge` object is the newer, scoped way to read live platform state; the hooks remain the API for auth state and the classic checkout flow, covered in the [Auth](/auth/) and [How billing works](/billing/how-it-works/) guides. Use whichever fits; they don't conflict.
