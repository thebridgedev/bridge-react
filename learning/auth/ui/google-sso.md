...
| `icon` | `ReactNode` | (none) | Custom icon element |
...
function SsoButtons({ connections }: { connections: FederationConnection[] }) {
  return (
    <>
...
In redirect mode, `onSuccess` / `onLogin` do **not** fire on the original page; it's already navigating away. Instead, rely on your route guard + `authState` transitions to pick up the session once the user lands back in your app.
