...
description: Standalone SSO login button for React.
sidebar:
  label: React
---
...
```tsx
import { SsoButton } from '@nebulr-group/bridge-react';
import type { FederationConnection } from '@nebulr-group/bridge-react';

function SsoRow({ connections }: { connections: FederationConnection[] }) {
  return (
    <>
...
|------|--------------|-------------|
| `'redirect'` **(default)** | Clicking the button navigates the current tab to the Bridge federation endpoint. The user is sent to the provider (Google, Microsoft, etc.), signs in, and the OAuth callback chain returns them to your app via `<CallbackHandler>`. No popup, no cross-window messaging. | Almost all apps. This is the safest, most compatible default — pop-up blockers, mobile browsers, embedded webviews, and strict CSPs all work out of the box. |
...
In redirect mode, `onSuccess` / `onLogin` do **not** fire on the original page — it's already navigating away. Instead, rely on `useAuth()`'s `authState` (via `<ProtectedRoute>` or your own check) to pick up the session once the user lands back in your app.
