...
> **Framework note:** `useAuth()` also returns a `logout()` that only clears the tokens and resets local auth state, without redirecting. Use that one when you want to stay in control of navigation and route with your router afterwards (for example `navigate('/')` from react-router).
