...
Role and privilege are both available as feature flag targeting attributes automatically, decoded from the JWT — no wiring, no context you have to pass by hand. For more on flags and rules in general, see [Feature flags](/feature-flags/feature-flags/).
...
function BetaReportsGate() {
  const { value } = useFlag('beta_reports', false);
