# Bridge React — Branding

You are applying a workspace's branding (logo, name, colours) inside a React application that uses The Bridge.
...
`bridge.app.branding` is a Bridge readable; subscribe to it with the `useBridgeReadable` hook:
...
export function Header() {
  ...
  if (!branding) return null; // null until the first snapshot lands

  return (
    <header
      style={{
        background: branding.bgColor ?? 'var(--surface)',
        color: branding.textColor ?? 'inherit',
      }}
    >
