// bridge-nextjs/flags — instance registry + change-notification bus.
//
// Plain TS (no React, no DOM), so it's safe to import from any code path —
// `bootstrap.ts`, the `useFlag` hook, and the `<FeatureFlag>` component all
// subscribe here. Ported near-verbatim from bridge-svelte's
// `src/lib/flags/registry.ts` (the file is framework-agnostic already; only
// the `.js` import extensions were dropped for the nextjs/tsc setup).
...
/**
 * Non-reactive flag read. Use inside `useFlag` (which subscribes to
 * `subscribeToFlagChanges` via `useSyncExternalStore`) if you need
 * reactivity from a component context.
 *
