/**
 * Live Channel Unification (TBP-289/331) — `bridge.events.handle({...})`
 * multiplex dispatcher.
 *
 * Ported verbatim from bridge-svelte's `core/events.ts` — this module is pure
 * TypeScript with no framework primitives, so it copies across unchanged.
 *
 * One API for every channel-event kind. Discriminated union by `kind`:
 *
 *   const unsub = bridge.events.handle({
 *     'flag.updated':          (m) => logger.debug('flag', m.flag.key),
 ...
 *   unsub();
 *
 * Multi-handler — multiple consumers can register handlers for the same kind;
 * all fire on every matching event. Handler errors are isolated: one throwing
 * handler doesn't block the rest, doesn't break the dispatch loop, doesn't
 * propagate out of `_dispatch`.
 *
 * The fallback `'*'` handler fires when no specific handler is registered for
 * that kind (NOT for every event — that would double-fire when both a specific
 * and a `*` handler are registered).
 */
import type {
  FlagUpdateMessage,
  FlagRemovedMessage,
...
/**
 * Discriminated handler table. Every kind in `RealtimeMessage`'s union is a
 * key; `'*'` is the catch-all when no specific handler is registered. Each
 * handler is optional; consumers register the subset they care about.
 */
...
export class BridgeEventsDispatcher {
  ...
  /**
   * Internal — called by the bridge runtime from every RealtimeClient hook.
