 ...
 * Ported from bridge-svelte (TBP-604) for TBP-721.
 *
 ...
 * single-worker run is byte-for-byte the setup this suite had before per-worker apps.
 */
...
export function workerAppOwnerEmail(parallelIndex: number): string {
  return `iman+pw-react-app-w${parallelIndex}@nebulr.group`;
}
